...
'''
Environment Variables: USER_ID, REDIS_PASS, CONSUMER_KEY, CONSUMER_SECRET, KEY, 
                        SECRET, MIDWEST
'''
...
def weekly_scores():
    client = redis.Redis(host=os.getenv("REDIS_HOST"), port=6379, db=4,
...
def set_active_players():
    ...
    for roster in rosters:
        ...
        client.delete(name)
        client.sadd("midwest_teams", name)
        team_list = []
...
def tweet_scores(client, num_matchups):
    ...
    week = get_week()
    beginning = f"MidWest - week {week} results: \n\n"
